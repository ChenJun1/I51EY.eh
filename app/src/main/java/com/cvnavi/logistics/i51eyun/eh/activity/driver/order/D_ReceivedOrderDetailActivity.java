...
@ContentView(R.layout.d_received_order_detail)
public class D_ReceivedOrderDetailActivity extends BaseActivity {
	...
	@ViewInject(R.id.d_CalAcceptOrdRule)
	private TextView d_CalAcceptOrdRule;
...
//	private TextView Completed_Orders_Time;
	...
	@ViewInject(R.id.cancle_info_layout)
	private LinearLayout cancle_info_layout;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().getSerializableExtra(Constants.OrderInfo) != null) {
			...
			if (waiOrderBaseInfo.Operate_Code.equals("AB")) {
				c_cancle_btn.setVisibility(View.VISIBLE);
				cancle_info_layout.setVisibility(View.VISIBLE);
			} else {
				c_cancle_btn.setVisibility(View.GONE);
				cancle_info_layout.setVisibility(View.GONE);
			}
		}
	...
	private void loadData() {
		loadReceivedOrderData(Constants.OrderInfo_Url);
	}
	...
	public void loadReceivedOrderData(final String URL) {
	...
	public void cancelReceivedOrderData() {
		new Thread(new Runnable() {
			@Override
			public void run() {
				RequestParams params = new RequestParams(Constants.Set_CanselOrderInfo_URL);
				try {
					// OrderRequestBean orderRequestBean = new
					// OrderRequestBean();
					// orderRequestBean.Order_Key = waiOrderBaseInfo.Order_Key;
				...
				x.http().post(params, new Callback.CommonCallback<String>() {
					...
					public void onSuccess(String str) {
						...
						try {
							...
							Message msg = Message.obtain();
							if (resultBase.isSuccess()) {
								msg.what = Constants.Request_ReceivedCancelOrder;
								myHandler.sendMessage(msg);
							} else {
								msg.what = Constants.Request_Fail;
	...
	@Event(value = { R.id.back_linearLayout, R.id.operation_btn, R.id.c_Consignee_Tel, R.id.c_Shipper_Tel,
			R.id.TH_Photo, R.id.JH_Photo, R.id.c_cancle_btn }, type = View.OnClickListener.class)
	private void onClick(View v) {
		...
		case R.id.JH_Photo:
			...
			showActivity(this, intent);
			break;
		case R.id.c_cancle_btn: // 提/交货后就不允许取消接单。
			if (TextUtils.isEmpty(MyApplication.getInstatnce().getLoginData().getWorkStatus()) == false && Integer
				...
				alertDialog.setMessage("您还未上班，不能取消接单！");
			...
			alertDialog = new AlertDialog.Builder(context);
			alertDialog.setTitle("提示");
			alertDialog.setMessage("确认取消接单吗？");
			alertDialog.setPositiveButton("确认", new OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
	...
	private Handler myHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Constants.Request_Success:
				...
				break;
			case Constants.Request_ReceivedCancelOrder:
				Toast.makeText(context, "取消接单成功！", Toast.LENGTH_SHORT).show();
				finish();
				break;
			case Constants.Request_NoData:
				Toast.makeText(context, "加载数据为空！请联系客服！", Toast.LENGTH_SHORT).show();
				break;
			case Constants.Request_Fail:
				alertDialog = new AlertDialog.Builder(D_ReceivedOrderDetailActivity.this);
				alertDialog.setCancelable(false);
				alertDialog.setTitle("提示");
	...
	private void setViewValue() {
		...
		c_Shipper_Name_x
				.setText(TextUtils.isEmpty(orderDetailInfo.Shipper_Name) == true ? "--" : orderDetailInfo.Shipper_Name);
		c_ReleaseTime.setText(
				TextUtils.isEmpty(orderDetailInfo.ReleaseTime) == true ? "--" : orderDetailInfo.ReleaseTime + "");

		c_ReceivingTime.setText(
				TextUtils.isEmpty(orderDetailInfo.ReceivingTime) == true ? "--" : orderDetailInfo.ReceivingTime);
		
		if(!TextUtils.isEmpty(orderDetailInfo.CalAcceptOrdRule)){
			double CalShipperRule=Double.valueOf(orderDetailInfo.CalAcceptOrdRule);
			d_CalAcceptOrdRule.setText("该运单可以在（提货时间-接单时间）*"+(CalShipperRule*100)+"%前取消");
		}

		// if (TextUtils.isEmpty(orderDetailInfo.IsTHPhoto) == false &&
		// orderDetailInfo.IsTHPhoto.equals("1")) {
		...
		// if (TextUtils.isEmpty(orderDetailInfo.IsJHPhoto) == false &&
		// orderDetailInfo.IsJHPhoto.equals("1")) {
