...
@ContentView(R.layout.d_car_location)
public class D_CarLocationActivity extends BaseActivity {
	...
	OnGetRoutePlanResultListener listener = new OnGetRoutePlanResultListener() {
		...
		public void onGetDrivingRouteResult(DrivingRouteResult result) {
			...
			if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
				Toast.makeText(D_CarLocationActivity.this, "抱歉，未找到结果",
	...
	private Handler myHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Constants.Request_NoData:
				Toast.makeText(D_CarLocationActivity.this, "没有更多数据！",
						Toast.LENGTH_SHORT).show();
				break;
			case Constants.Request_Fail:
				Toast.makeText(D_CarLocationActivity.this, "数据加载失败！",
						Toast.LENGTH_SHORT).show();
				break;
			default:
				break;
...
//			showPop(Consignee,"收货地址");
	...
	private void showPop(LatLng position, String makTitle) {
		...
		if (makTitle.equals("发货地址")) {
			InfoWindow mInfoWindow1 = new InfoWindow(popup, position, listener1);
			baiduMap.showInfoWindow(mInfoWindow1);
		}
		if(makTitle.equals("收货地址")){
			InfoWindow mInfoWindow2 = new InfoWindow(popup, position, listener1);
